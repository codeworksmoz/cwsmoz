# Codworks MOZ - Plataforma de Educação em Programação
...
2.  **Configure as Variáveis de Ambiente:**
    ...
    - Preencha os valores em `.env.local` com suas credenciais do Supabase. Este arquivo não será versionado.
...
4.  Abra [http://localhost:9002](http://localhost:9002) no seu navegador para ver a aplicação.
...
O projeto está pronto para ser implantado em plataformas modernas de hospedagem de frontend.

-   **Vercel:** A implantação é "zero-config". Basta importar seu repositório Git no Vercel, configurar as variáveis de ambiente no painel de controle e fazer o deploy.
-   **Netlify:** A implantação também é simplificada. O arquivo `netlify.toml` incluído no projeto configura o processo de build automaticamente. Você só precisa configurar as variáveis de ambiente no painel do Netlify.
